...
Funcion MENUDELPACIENTE(Nombre Por Referencia, Apellido Por Referencia, DNI Por Referencia, Correo Por Referencia, i Por Valor)
	...
	nom<-Nombre[i] ; ape<-Apellido[i] ; doc<-DNI[i] ; email<-Correo[i] 
Repetir
	Limpiar Pantalla
	Escribir " -----------------                                              -----------------"
...
Hasta Que (OPCIONMENU>=1 O OPCIONMNU<=5)
		Segun OPCIONMENU Hacer
			...
			2:	
				MISTURNOS();
			3:	
				INFORMACIONPERSONAL(nom,ape,doc,email)
			4: 
				INFORMACIONDELAAPP();
...
Funcion INFORMACIONPERSONAL(nombre Por Valor, apellido Por Valor, dni Por Valor, correo Por Valor) //FALTA LOS ARGUMENTOS
...
FinFuncion
	...
	Funcion MENUDELDOCTOR(nombres, apellidos, i Por Referencia)
		Definir OPCIONMENU Como Entero
		Limpiar Pantalla
		Repetir
