...
Funcion CargarDoctores
		...
		lista_profesionales(nombres, apellidos, cedula_medica, especialidades, limite)
	FinFuncion

	Funcion lista_profesionales(nombres , apellidos , cedula_medica , especialidades , limite Por Valor)
		Definir opc, cedula, j, confirmacion Como Entero
			confirmacion=0
			Borrar Pantalla
			Escribir " --------------------"
			...
			Para  j=1 hasta limite con paso 1 Hacer
				si (cedula == cedula_medica[j] )
					Escribir " ¡MEDICO ENCONTRADO! "
					Escribir "Bienvenido/a doctor/a ", nombres[j], " ", apellidos[j]
					confirmacion<-1
				SiNo
					Repetir
						Borrar Pantalla
			...
			si confirmacion == 1 Entonces
				MENUDELDOCTOR(nombres,apellidos,cedula_medica,limite,j)
			FinSi
	FinFuncion
